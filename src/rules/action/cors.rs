...
pub fn apply_req(req: &Request<'_>, resp: &mut Response<'_>, opts: &CorsOpts) {
    ...
    let origin_value = origin.clone_string();
    if !valid_origin(&origin_value, &opts.origins) {
        return;
    }
    resp.headers
        .add("Access-Control-Allow-Origin", origin_value);

    if let Some(max_age) = opts.max_age {
        resp.headers.add("Access-Control-Max-Age", max_age);
    }
    ...
    if !opts.methods.is_empty() {
        ...
        resp.headers
            .add("Access-Control-Allow-Methods", methods_value);
    }
